...
class Harness:
    ...
    def run(self, initial_goal_prompt_or_file: str):
        ...
        # Ensure log directory has a README for analysis
        ...
        while (
            ...
            try:
                ...
                logging.error(f"Failed to create iteration log file {iter_log_path}: {e}")
            ...
            try:
                ...
                if verdict == "SUCCESS":
                    ...
                    if self.enable_code_review:
                        ...
                        try:
                            import asyncio
                            asyncio.get_running_loop()
                            asyncio.create_task(self._send_ui_update({"status": "Running Code Review", "log_entry": "Running code review..."}))
                        except RuntimeError:
                            pass
                        try:
                            ...
                            try:
                                import asyncio
                                asyncio.get_running_loop()
                                asyncio.create_task(self._send_ui_update({"status": "Code Review Complete", "log_entry": "Code review completed and saved."}))
                            except RuntimeError:
                                pass
                            ...
                            try:
                                import asyncio
                                asyncio.get_running_loop()
                                asyncio.create_task(self._send_ui_update({"status": "Error", "log_entry": f"Code review failed: {review_err}"}))
                            except RuntimeError:
                                pass
                ...
                logging.info(f"=== ITERATION {iteration_num_display} ENDED ===")
                logging.info(f"Iteration {iteration_num_display} summary: "
                             f"converged={self.state.get('converged', False)}, "
                             f"last_error={self.state.get('last_error', None)}")
        ...
        # Log a summary at the end of the run for analysis
        logging.info("=== HARNESS RUN ENDED ===")
        logging.info(f"Final run state: {self.state}")
        logging.info(f"Run summary: converged={self.state.get('converged')}, "
                     f"iterations={self.state.get('current_iteration')}, "
