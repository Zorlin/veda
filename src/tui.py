...
class VedaApp(App[None]):
    ...
    def ask_initial_prompt(self) -> None:
        ...
        if not self.ollama_client:
            self.call_soon(self.log_widget.write, "[bold red]Cannot generate initial prompt: Ollama client not available.[/]")
            return

        initial_question = "What project goal should I work on today?"
        self.call_soon(self.log_widget.write, "[italic grey50]Veda is thinking about the first question...[/]")
        try:
            ...
            self.call_soon(self.log_widget.write, escaped_question)
            # Original line causing issues (potentially):
            # self.call_soon(self.log_widget.write, f"[bold magenta]Veda:[/bold] {initial_question}")
        except Exception as e:
            logger.exception("Error generating initial prompt:")
            escaped_error = rich.markup.escape(str(e))
            self.call_soon(self.log_widget.write, f"[bold red]Error generating initial prompt: {escaped_error}[/]")
        finally:
             # Ensure input is focused after the prompt is displayed
             self.call_soon(self.input_widget.focus)
    ...
    def call_ollama(self, prompt: str) -> None:
        ...
        if not self.ollama_client:
            ...
            self.call_soon(self.log_widget.write, "[bold red]Cannot process: Ollama client not available.[/]")
            return

        # Use call_soon for UI updates from worker
        self.call_soon(self.log_widget.write, "[italic grey50]Thinking...[/]")
        try:
            # Synchronous call within the worker thread
            response = self.ollama_client.generate(prompt)
            # Update UI from the worker thread safely
            self.call_soon(self.log_widget.write, f"[bold magenta]Veda ({self.ollama_client.model}):[/] {response}")
        except Exception as e:
            # Log the exception and display an error in the TUI
            logger.exception("Error during Ollama call in worker thread:")
            escaped_error = rich.markup.escape(str(e))
            self.call_soon(self.log_widget.write, f"[bold red]Error during Ollama call: {escaped_error}[/]")
        finally:
            # Ensure input is cleared and focused even if there was an error
            self.call_soon(self.input_widget.clear)
            self.call_soon(self.input_widget.focus)
